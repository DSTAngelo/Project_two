...
function inicioDeParametros(id) {
  document.querySelector(id).style.display = 'flex';
}


function desabilitar(x) {
  const cajaTexto = document.querySelector(x);
...
function evaluacion() {
  ...
  document.querySelector("#evaluar").addEventListener("click", () => {
    ...
    for (const i in palabra_Buscar) {
      ...
      if (oculto.indexOf("_") < 0) {
          console.log(oculto);
          alert("Has Ganado");
          desabilitar("#caracter");
          desabilitar("#evaluar");
          inicioDeParametros("#seis");
          inicioDeParametros("#lineaGanador");
          break;
      }
    }
    ...
    if (aprobado) {
      ...
      if (contadorErrores == 4) {
        ...
        desabilitar("#evaluar");
        inicioDeParametros("#seis");
        inicioDeParametros("#lineaPerdedor");
      }
    }
    document.querySelector("#salidaLetra").innerHTML = oculto;
    limpiarCaja();
